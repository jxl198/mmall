...
public class UserServiceImpl implements IUserService {
    ...
    public ServerResponse<String> register(User user) {
        ServerResponse serverResponse = checkValid(user.getUsername(), Const.USER_NAME);
    ...
    public ServerResponse<String> forgetResetPassword(String username, String passwordNew, String forgetToken) {
        ...
        ServerResponse validResoponse = checkValid(username, Const.USER_NAME);
    ...
    public ServerResponse<String> resetPassword(String passwordOld, String passwordNew, User user) {
        //防止横向越权， 校验这个用户的旧密码，一定要指定是这个用户，因为我们查询 一个count（1） ，如果不指定id，那么结果是true
    ...
    public ServerResponse<User> updateInformation(User user) {
        //username 不能被更新
        //email 校验新的email是不是已经存在，并且存在的email不能是当前用户的
